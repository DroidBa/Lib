...
public abstract class CropImage implements Transformation {
  ...
  public void computeImageTransformation() {
    int viewWidth =
        cropImageView.getWidth() - cropImageView.getPaddingLeft() - cropImageView.getPaddingRight();
    int viewHeight = cropImageView.getHeight()
    ...
    if (cropType != CropType.NONE && viewHeight > 0 && viewWidth > 0) {
      ...
      float xTranslation =
          getXTranslation(cropType, viewWidth, postDrawableWidth, verticalImageMode);
      float postDrawabeHeigth = drawableHeight * scale;
      float yTranslation =
          getYTranslation(cropType, viewHeight, postDrawabeHeigth, verticalImageMode);
  ...
  private float getYTranslation(@CropType int cropType, int viewHeight, float postDrawabeHeigth,
  ...
  private float getXTranslation(@CropType int cropType, int viewWidth, float postDrawableWidth,
      boolean verticalImageMode) {
    if (!verticalImageMode) {
      switch (cropType) {
